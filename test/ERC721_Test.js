...
contract('ERC721Token',accounts=>{
    ...
    beforeEach( async function(){
            this.contract=await ERC721Token.new({from:defaultUser});
    })
    ...
    describe('can create a token',()=>{
        ...
        beforeEach(async()=>{
            txn=await this.contract.mint(tokenId,{from:user1});
        });

        it('owner of token is user1 ',async function(){
            assert.equal(await this.contract.ownerOf(tokenId),user1);
        })

        it('Balance of user 1', async  function (){
        ...
        it('emits the correct event during creation of a new token', async function () { 
            assert.equal(txn.logs[0].event, 'Transfer')
        })
